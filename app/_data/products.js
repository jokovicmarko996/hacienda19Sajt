...
export function getProductById(id) {
  ...
  // pokušaj kao broj (ako su id u data kao brojevi)
  const asNumber = Number.parseInt(raw, 10);
  const byNumber =
    Number.isFinite(asNumber) && products.find((p) => p.id === asNumber);
  if (byNumber) return byNumber;

  // fallback (ako su id kao stringovi)
  const byString = products.find((p) => String(p.id) === raw);
